  ...
  <div>
    ...
    <div style="width:85%;margin: 0 auto;margin-bottom: 50px;">
        <div style="padding-bottom: 15px ;margin-top: 20px;text-align: left;">
          <span style="font-size: 14px;margin-right: 20px;">当前位置：首页 > 我的视频收藏</span>
        </div>

        <div style="padding-bottom: 15px ;border-bottom: 3px solid #333333; text-align: left;display: flex;">
          <span style="font-weight: bold; font-size: 24px;float: left;flex: 3;color: #333333;">我的视频收藏</span>
        </div>
      ...
      <el-table-column label="视频"><template #default="scope"><span v-if="scope.row.vedioId" @click="router.push('/front/vedio-detail?id=' + scope.row.vedioId)" style="cursor: pointer;">{{ state.vedioOptions.find(v => v.id === scope.row.vedioId) ? state.vedioOptions.find(v => v.id === scope.row.vedioId).name : '' }}</span></template></el-table-column>
