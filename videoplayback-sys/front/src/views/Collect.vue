...
<template>
  <div>
      ...
      <el-table-column label="视频"><template #default="scope"><span v-if="scope.row.vedioId">{{ state.vedioOptions.find(v => v.id === scope.row.vedioId) ? state.vedioOptions.find(v => v.id === scope.row.vedioId).name : '' }}</span></template></el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('collect.edit')">修改<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
    ...
    <el-dialog v-model="dialogFormVisible" title="视频收藏信息" width="40%">
